...
| `CCP_COMPACT_EFFORT` | none | `low` | Caps Codex reasoning effort for Claude Code summary compaction requests. A compaction request with no effort uses the cap. `off` disables the cap and `none` removes reasoning. |
...
| `CCP_CODEX_HEADER_TIMEOUT_MS` | `codex.headerTimeoutMs` | `300000` | How long an HTTP-transport request waits for Codex response headers, in milliseconds. Values below `1000` are ignored. |
