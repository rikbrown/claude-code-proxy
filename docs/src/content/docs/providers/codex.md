...
- **Checked prefix:** the producing request's conversation items followed by every output item of that turn (assistant text, tool calls, and reasoning). A later request replays the item only if its translated input starts with exactly this prefix. An edited, regenerated, or competing reply therefore never passes the check.
- **Stripped prefix:** the request's conversation items plus only the output items emitted before the `compaction` item. This is the part the proxy replaces with the item on the wire; the rest of the checked prefix and everything after it stay explicit. When the item also holds a reply that was emitted after it, that reply is sent twice, which is wasteful but never wrong.

Claude Code still sends its complete history; only the upstream request is shortened. If the proxy cannot establish either record, for example because the item cannot be positioned against the turn's output or an output item has no exact client round-trip form, it stores nothing and the next turn sends the full history.
